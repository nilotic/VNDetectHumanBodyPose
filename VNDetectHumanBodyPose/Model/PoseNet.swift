...
protocol PoseNetDelegate: class {
...
final class PoseNet {
    ...
    init?() {
        guard let model = try? PoseNetMobileNet075S16FP16(configuration: .init()).model else { return nil }
        poseNetMLModel = model
    }
    ...
    func predict(_ image: CGImage) {
        DispatchQueue.global(qos: .userInitiated).async {
            // Wrap the image in an instance of PoseNetInput to have it resized before being passed to the PoseNet model.
            ...
            DispatchQueue.main.async { self.delegate?.poseNet(self, didPredict: poseNetOutput) }
        }
    }
}
