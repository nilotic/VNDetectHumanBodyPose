...
struct PoseBuilder {
    ...
    init(output: PoseNetOutput, configuration: PoseBuilderConfiguration, inputImage: CGImage) {
        ...
        modelToInputTransformation = CGAffineTransform(scaleX: inputImage.size.width / output.modelInputSize.width, y: inputImage.size.height / output.modelInputSize.height)
    }
}
